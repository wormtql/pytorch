...
LazyTensor IndexAcrossDims(const LazyTensor& input, lazy_tensors::int64 dim,
  ...
  return LazyTensorAtenOp::squeeze(
      LazyTensorAtenOp::slice(input, dim, index, index + 1, 1), dim);
}
...
LazyTensor Cross(const LazyTensor& input, const LazyTensor& other,
  ...
  at::Scalar one(1);
  LazyTensor s1 = LazyTensorAtenOp::sub(LazyTensorAtenOp::mul(u2, v3),
                                        LazyTensorAtenOp::mul(u3, v2), one);
  LazyTensor s2 = LazyTensorAtenOp::sub(LazyTensorAtenOp::mul(u3, v1),
                                        LazyTensorAtenOp::mul(u1, v3), one);
  LazyTensor s3 = LazyTensorAtenOp::sub(LazyTensorAtenOp::mul(u1, v2),
                                        LazyTensorAtenOp::mul(u2, v1), one);
  // Stack the terms into one result tensor.
  return LazyTensorAtenOp::stack({s1, s2, s3}, canonical_dim);
}
...
LazyTensor KlDivBackward(const LazyTensor& grad_output, const LazyTensor& input,
                         ...
                         bool log_target) {
  auto input_shape_ref = input.shape();
  LazyTensor expanded_grad_output = LazyTensorAtenOp::expand(
      grad_output, lazy_tensors::util::ToVector<lazy_tensors::int64>(
                       input_shape_ref.get().dimensions()));
  LazyTensor grad_input;
  if (!log_target) {
    grad_input = LazyTensorAtenOp::where(
        LazyTensorAtenOp::gt(target, 0),
        LazyTensorAtenOp::neg(
            LazyTensorAtenOp::mul(target, expanded_grad_output)),
        LazyTensorAtenOp::full_like(input, 0, input.GetDevice(), c10::nullopt));
  } else {
    grad_input = LazyTensorAtenOp::neg(LazyTensorAtenOp::mul(
...
LazyTensor MakeMatrixWithDiagonal(const LazyTensor& input,
  ...
  LazyTensor identity =
      LazyTensorAtenOp::eye(size, size, input.GetDevice(), input.dtype());
  auto padding =
      diagonal >= 0
          ? std::vector<lazy_tensors::int64>{diagonal, 0, 0, diagonal}
          : std::vector<lazy_tensors::int64>{0, -diagonal, -diagonal, 0};
  return LazyTensorAtenOp::constant_pad_nd(
      LazyTensorAtenOp::mul(identity, input), padding, 0);
}
...
LazyTensor SmoothL1Loss(const LazyTensor& input, const LazyTensor& target,
  ...
  auto broadcasted_inputs =
      LazyTensorAtenOp::broadcast_tensors({input, target});
  ...
  LazyTensor diff =
      LazyTensorAtenOp::sub(broadcasted_input, broadcasted_target, one);
  at::Scalar half(0.5);
  at::Scalar half_beta(0.5 * beta);
  LazyTensor abs_diff = LazyTensorAtenOp::abs(diff);
  LazyTensor squared_loss = LazyTensorAtenOp::div(
      LazyTensorAtenOp::mul(LazyTensorAtenOp::mul(diff, diff), half),
      beta_scalar);
  LazyTensor l1_loss = LazyTensorAtenOp::sub(abs_diff, half_beta, one);
  LazyTensor elementwise_loss = LazyTensorAtenOp::where(
      LazyTensorAtenOp::lt(abs_diff, beta_scalar), squared_loss, l1_loss);
  ...
  switch (reduction) {
    ...
    case ReductionMode::kSum:
      return LazyTensorAtenOp::sum(elementwise_loss, all_dimensions, false,
                                   broadcasted_input.dtype());
    default:
...
LazyTensor SmoothL1LossBackward(const LazyTensor& grad_output,
  ...
  LazyTensor diff =
      LazyTensorAtenOp::sub(broadcasted_input, broadcasted_target, one);
  LazyTensor abs_diff = LazyTensorAtenOp::abs(diff);
  LazyTensor grad_squared_loss = LazyTensorAtenOp::div(
      LazyTensorAtenOp::sub(broadcasted_input, broadcasted_target, one),
      beta_scalar);
  LazyTensor ones = LazyTensorAtenOp::full_like(broadcasted_input, one,
                                                broadcasted_input.GetDevice(),
                                                broadcasted_input.dtype());
  // NB: We can't use LazyTensorAtenOp::sign(), it returns zero for input zero.
  LazyTensor grad_l1_loss = LazyTensorAtenOp::where(
      LazyTensorAtenOp::gt(broadcasted_input, broadcasted_target), ones,
      LazyTensorAtenOp::neg(ones));
  LazyTensor elementwise_loss_backward =
      LazyTensorAtenOp::where(LazyTensorAtenOp::lt(abs_diff, beta_scalar),
  ...
  switch (reduction) {
    ...
    case ReductionMode::kMean: {
      ...
      return LazyTensorAtenOp::mul(
          LazyTensorAtenOp::div(elementwise_loss_backward, grad_scale),
          grad_output);
    }
    default:
      LTC_ERROR() << "Invalid reduction type: "
...
LazyTensor Softplus(const LazyTensor& input, const at::Scalar& beta,
  ...
  return LazyTensorAtenOp::where(
      LazyTensorAtenOp::gt(LazyTensorAtenOp::mul(input, beta), threshold),
      input,
      LazyTensorAtenOp::div(LazyTensorAtenOp::log1p(LazyTensorAtenOp::exp(
                                LazyTensorAtenOp::mul(input, beta))),
                            beta));
}
...
LazyTensor SoftplusBackward(const LazyTensor& grad_output,
  ...
  LazyTensor scaled_input = LazyTensorAtenOp::mul(input, beta);
  LazyTensor z = LazyTensorAtenOp::exp(LazyTensorAtenOp::mul(output, beta));
  return LazyTensorAtenOp::where(
      LazyTensorAtenOp::gt(scaled_input, threshold), grad_output,
      LazyTensorAtenOp::mul(
          grad_output,
          LazyTensorAtenOp::div(LazyTensorAtenOp::sub(z, 1, 1), z)));
}
...
LazyTensor Select(const LazyTensor& input, lazy_tensors::int64 dim,
  ...
  LazyTensor result = LazyTensorAtenOp::narrow(input, dim, index, 1);
  auto new_dims = Helpers::DropDimensions(shape.get().dimensions(), {dim});
  return LazyTensorAtenOp::view(result, new_dims);
}
...
LazyTensor EmbeddingDenseBackward(const LazyTensor& grad_output,
  ...
  LazyTensor grad =
      LazyTensorAtenOp::view(grad_output, {numel, grad_output.size(-1)});
  LazyTensor grad_weight =
      LazyTensorAtenOp::full({num_weights, grad_output.size(-1)}, 0,
                             grad_output.GetDevice(), grad_output.dtype());
  LazyTensor indices_rank1 = LazyTensorAtenOp::view(indices, {numel});
  if (scale_grad_by_freq) {
    // Compute the histogram of index values.
    LazyTensor counts = LazyTensorAtenOp::full(
        {num_weights}, 0, indices.GetDevice(), indices.dtype());
    LazyTensor ones = LazyTensorAtenOp::full({numel}, 1, indices.GetDevice(),
                                             indices.dtype());
    LazyTensorAtenOp::index_put_(
        counts, counts, {indices_rank1}, /*start_dim=*/0,
        /*values=*/ones,
        /*accumulate=*/true, /*result_permutation=*/{0});
    LazyTensor grad_weights_scale =
        LazyTensorAtenOp::index(counts, {indices_rank1}, 0);
    // Scale the value of the gradient by the histogram.
    grad = LazyTensorAtenOp::div(
        grad, LazyTensorAtenOp::unsqueeze(grad_weights_scale, 1));
  }
  // Don't accumulate gradients for indices which are equal with the given
  // padding_idx.
  LazyTensor skip_padding = LazyTensorAtenOp::unsqueeze(
      LazyTensorAtenOp::ne(indices_rank1, static_cast<double>(padding_idx)), 1);
  skip_padding = LazyTensorAtenOp::expand(
      skip_padding, lazy_tensors::util::ToVector<lazy_tensors::int64>(
                        grad.shape().get().dimensions()));
  LazyTensor zero_grad =
      LazyTensorAtenOp::full_like(grad, 0, grad.GetDevice(), grad.dtype());
  return LazyTensorAtenOp::index_put(
      grad_weight, {indices_rank1},
      /*start_dim=*/0,
      /*values=*/LazyTensorAtenOp::where(skip_padding, grad, zero_grad),
      /*accumulate=*/true,
