...
std::shared_ptr<ir::Value> AllReduceInPlace(
  ...
  return std::make_shared<ir::Value>(LazyTensorDistributed::all_reduce(
...
std::pair<at::Tensor, std::shared_ptr<ir::Value>> AllReduce(
  ...
  LazyTensor result;
  ir::Value new_token;
  std::tie(result, new_token) = LazyTensorDistributed::all_reduce(
...
std::pair<at::Tensor, std::shared_ptr<ir::Value>> AllToAll(
  ...
  LazyTensor result;
  ir::Value new_token;
  std::tie(result, new_token) = LazyTensorDistributed::all_to_all(
...
std::pair<at::Tensor, std::shared_ptr<ir::Value>> CollectivePermute(
  ...
  LazyTensor result;
  ir::Value new_token;
  std::tie(result, new_token) = LazyTensorDistributed::collective_permute(
...
py::object LtcNms(const at::Tensor& boxes, const at::Tensor& scores,
  ...
  {
    NoGilSection nogil;
    auto nms_result = LazyTensorAtenOp::nms(
